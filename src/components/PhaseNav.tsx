...
import { FileText, LayoutList, Palette, LayoutGrid, Search, Package } from 'lucide-react'
import { loadLandingData, hasExportZip } from '@/lib/landing-loader'
import { getAllSectionIds, getSectionDesigns } from '@/lib/section-loader'

export type Phase = 'vision' | 'structure' | 'design' | 'sections' | 'seo' | 'export'
...
const phases: PhaseConfig[] = [
  { id: 'vision', label: 'Vision', icon: FileText, path: '/' },
  { id: 'structure', label: 'Structure', icon: LayoutList, path: '/structure' },
  { id: 'design', label: 'Design', icon: Palette, path: '/design' },
  { id: 'sections', label: 'Sections', icon: LayoutGrid, path: '/sections' },
  { id: 'seo', label: 'SEO', icon: Search, path: '/seo' },
...
function usePhaseStatuses(): PhaseInfo[] {
  ...
  const sectionsWithDesigns = useMemo(() => {
    return sectionIds.filter(id => getSectionDesigns(id).length > 0).length
  }, [sectionIds])
  const hasSections = sectionsWithDesigns > 0

  // Determine current phase from URL
  const currentPath = location.pathname
  let currentPhaseId: Phase = 'vision'

  if (currentPath === '/' || currentPath === '/vision') {
    currentPhaseId = 'vision'
  } else if (currentPath === '/structure') {
    currentPhaseId = 'structure'
      ...
      {phaseInfos.map(({ phase, status, isComplete }, index) => {
        ...
        return (
          <div key={phase.id} className="flex items-center">
            {!isFirst && (
              <div
                className={`w-4 sm:w-6 lg:w-10 h-px transition-colors duration-200 ${
                  status === 'upcoming'
